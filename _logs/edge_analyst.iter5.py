...
CRITICAL: the numeric fields (confidence, bet_size, tp_pct, sl_pct) MUST be computed from the formulas in this prompt using the market data you are given. Do NOT re-emit the example numbers shown here — those are illustrations. Two trades with different asks/spreads/confidences MUST have different TP/SL/bet values.

YOUR JOB: find high-quality BUY setups. Back-test shows round-level signals alone are only 51% accurate, so the spread eats us. BUY when MAP SCORE CONTEXT + ≥2 round signals agree. Default to SKIP when only round signals are present.

PRICE / SPREAD GATES (hard — outside this SKIP):
- ask must be 25¢–70¢
- ask=0 or bid=0 → SKIP (untradable)
- spread (ask-bid) must be ≤ 10¢

MAP SCORE CONTEXT (this is our #1 signal — weight heavily):
- 0-0 in maps BEFORE anyone has won map 1 = TIED, NOT trailing.
- 1-1 in maps = TIED, NOT trailing.
- BUY SIDE LEADING in maps (1-0, or 2-1 closeout): STRONG edge → BUY when ≥1 round signal agrees.
- BUY SIDE TIED in maps (0-0, 1-1): BUY when ≥2 round signals agree (round-score lead is nice but NOT required — "0-0 tied with eco advantage + kill lead" is a valid BUY).
- BUY SIDE TRAILING in maps (0-1, 0-2, 1-2): need BOTH ≥3 round-lead on CURRENT MAP and eco advantage. Otherwise SKIP — do not buy a trailing team on mid-round momentum alone.

TRIGGER EVENT RULES:
- round_end / map_win for buy side: strong trigger, evaluate all signals.
- open_kill / kill_streak / clutch_win for buy side: mid-round event, do NOT buy unless MAP-SCORE is leading-or-tied AND ≥2 other round signals agree.
- Event team is shown in the event list. If event fired for the OPPONENT, SKIP.

ROUND-LEVEL SIGNALS (count these — need ≥2 aligned for most BUYs):
 +1 economy advantage (buy side ≥30% more money)
 +1 kill lead (current round; ≥2 more kills)
 +1 alive advantage (e.g. 5v3 or 4v2)
 +1 HP/damage advantage (≥80hp more damage dealt this round)
 +1 AWP advantage (buy has, opponent doesn't)
 +1 buy-type mismatch (full_buy vs eco/pistol)
 +1 open_kill OR clutch_win just fired for buy side
 +1 favorable CT map-side (mirage/inferno/anubis defense)

SKIP cases (no BUY):
- ask outside 25¢-70¢ or spread > 10¢
- buy side trailing in MAPS with no eco advantage
- single round-signal only (e.g. "just won a round" alone)
- conflicting data (claim eco advantage but buy side has less money)

BET SIZING — COMPUTE from balance and confidence (do NOT default):
  stake_pct = round(0.025 + (confidence - 0.55) * 0.12, 3)
  bet_size  = round(balance * stake_pct, 0)
  Examples (balance=$1000):
    conf 0.60 → 3.1% → $31
    conf 0.70 → 4.3% → $43
    conf 0.80 → 5.5% → $55
    conf 0.90 → 6.7% → $67
  CAPS: min $25, max 8% of balance (whichever binds)
  DEAD-ZONE ask 0.45-0.55: multiply stake_pct by 0.6
  BALANCE < $600: multiply stake_pct by 0.5

TP/SL — COMPUTE from ask and spread (do NOT default — bad data kills trades):
  upside = (0.97 - ask) / ask      # realistic ceiling before resolution
  tp_pct = round(clamp(upside * 0.55, 0.04, 0.14), 3)
  sl_pct = round(clamp(spread + 0.10, 0.12, 0.20), 3)
  SKIP if upside < 0.08 (not enough room after spread cost).
  Examples:
    ask=0.40 spr=0.04 → upside=1.43 → tp≈14%  sl≈14%
    ask=0.60 spr=0.05 → upside=0.62 → tp≈14%  sl≈15%
    ask=0.75 spr=0.06 → upside=0.29 → tp≈16%→clamp 14%  sl≈16%
    ask=0.82 spr=0.05 → upside=0.18 → tp≈10%  sl≈15%
    ask=0.90 spr=0.04 → upside=0.08 → SKIP (not enough room)

REASON REQUIREMENTS (short, <180 chars):
- Include MAP SCORE (e.g. "leads 1-0 in maps")
- Include at least 2 specific numbers from the prompt (economy $, kills, alive, HP, ¢)
- Use actual team names — never "BUY_TEAM"

HALLUCINATION BANS (auto-SKIP):
- Never cite "Kambi" or external bookmakers.
- Don't invent numbers not in the prompt.
- If buy side has LESS money, that is a disadvantage (not advantage).
- If reason contradicts data (e.g. claims "kill lead" but kills_b > kills_a), SKIP.

MOMENTUM SANITY:
- 10s ±15¢ or 60s ±25¢ spikes on thin book = noise, not signal.
- momentum = 0.0¢ is FLAT (not positive).
- Real momentum: +1-5¢ over 30-60s with multiple trades."""
...
class EdgeAnalyst:
    ...
    def _build_prompt(self, events: list[dict], game: dict, market: dict) -> str:
        ...
        # Include ALL 6 fields with placeholders so qwen doesn't default to
        # cached example values from the system prompt.
        buy_team = game.get("buy_team", game.get("team_a", "?"))
        lines.append("")
        lines.append(f"DECIDE on BUY {buy_team}. Compute each numeric field from the formulas above — do NOT reuse example numbers.")
        lines.append("Reply with ONLY this JSON (fill <...> from your computation):")
        lines.append('{"action":"buy"|"skip","confidence":<0.55-0.95>,"bet_size":<computed $>,"tp_pct":<computed 0.04-0.14>,"sl_pct":<computed 0.12-0.20>,"reason":"<brief, include map-score + 2 signals>"}')
