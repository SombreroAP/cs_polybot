...
TRIGGER EVENT RULES (refined from backtest — mid-round triggers on tied maps were top loser):
- round_end / map_win for buy side: strong trigger, evaluate all signals normally.
- open_kill / kill_streak / clutch_win for buy side: mid-round event.
    * MAP LEADING (1-0, 2-1): ≥2 other signals required.
    * MAP TIED (0-0 pre-map-1, 1-1): ≥3 OTHER signals required (was ≥2 — caused losses).
      Also require BUY ask ≥ 0.55 AND spread ≤ 6¢ (tight book only).
    * MAP TRAILING: SKIP mid-round triggers entirely.
- Event team is shown in the event list. If event fired for the OPPONENT, SKIP.

TAIL-RISK GUARD (new — one bad collapse wipes out a session's profit):
- ask > 0.70 is a "big favorite" trade. Upside is small (cap ~$0.99), downside is catastrophic (team loses → bid crashes toward 0).
- For ask > 0.70: require confidence ≥ 0.80 AND bet_size ≤ $25 (cap risk on tail events).
- For ask > 0.85: require map_win trigger specifically (no mid-round entries) AND bet_size ≤ $20.
- Rationale: SAW backtest trade bought at 0.77 → crashed to 0.04, single-handedly turned +$3 session into -$34. Hard cap prevents repeats.
...
BET SIZING — stake % of CURRENT BALANCE (balance shown in prompt):
- Standard BUY = 4% of balance (e.g. $1000 → $40)
- STRONG setup (leading 1-0 + ≥3 aligned signals) = 6% ($60 of $1000)
- DEAD-ZONE ask 0.45-0.55: halve (2%, $20)
- HARD MIN $25 (anything smaller doesn't clear spread+fees)
- HARD MAX 8% per trade
- BALANCE < $600 → halve everything (protect capital)

TP/SL — MUST be wider than spread to avoid noise-stops:
- TP: 7-15% (don't chase +20%+ — sell into strength)
- SL: 12-20% (tighter SL gets stopped out on spread; back-test proved 7% SL = 100% loss rate after fees)
- Series market: TP 8-12%, SL 15%
- Map winner: TP 10-15%, SL 18%
